...
function createBatches(
	...
	for (const b of blocks) {
		...
		if (cur.length >= maxItems || chars + b.text.length > maxChars) {
...
async function runWithConcurrency<T>(
	...
	async function worker() {
		while (queue.length) {
			...
			if (!t) break;
			try {
				results.push({ status: "fulfilled", value: await t() });
			} catch (e) {
...
async function main() {
	...
	const pdfPath = getArg(
		"pdf",
		"docs/test/fixtures/한국기술대_전자결재_시스템고도화.pdf",
	);
	const timeoutMs = parseInt(getArg("timeout", "60000"), 10);

	if (!process.env.LLM_API_KEY) {
		console.error("❌ LLM_API_KEY 미설정");
		process.exit(1);
	}
	...
	const startPos = Math.max(
		0,
		(detected.find((d) => d.position > text.length * 0.08)?.position || 0) -
			200,
	);
	const excerpt = text.slice(startPos, Math.min(text.length, startPos + 20000));
	const blocks = selectBestCandidates(createIdBoundaryBlocks(excerpt)).filter(
		(b: any) => b.expectedId,
	);
	console.log(
		`   텍스트: ${text.length}자, ID: ${ids.length}개, 블록: ${blocks.length}개\n`,
	);
	...
	for (const batchSize of batchSizes) {
		for (const concurrency of concurrencies) {
			for (const model of models) {
				...
				console.log(
					`🚀 ${model} batch=${batchSize} concur=${concurrency} (${batches.length}개 배치)`,
				);
				...
				const tasks = batches.map((batch) => async () => {
					const input: RequirementBatchInput = {
						requirements: batch.items.map((b: any) => ({
							id: b.expectedId,
							text: b.text,
						})),
					};
					const output = await enrichBatch(client, model, input, {
						timeoutMs,
						onDiagnostic: (m: DiagnosticMeta) => {
							console.log(
								`  ${m.errorType ? "⚠️" : "✅"} ${formatDiagnostic(m)}`,
							);
							m.errorType ? fail++ : ok++;
						},
					});
					...
					if (output.usage) {
						totalTk += output.usage.totalTokens;
						promptTk += output.usage.promptTokens;
						compTk += output.usage.completionTokens;
					}
				});
				...
				const cost =
					(promptTk / 1_000_000) * price.input +
					(compTk / 1_000_000) * price.output;
				...
				console.log(
					`   → ${extracted.size}/${ids.length}개, ${(elapsed / 1000).toFixed(1)}s, ${totalTk} tokens, $${cost.toFixed(4)}\n`,
				);
			}
		}
	}
	...
	console.log(
		"Model           Batch Concur Batches Ok/Fail    IDs     Time   Tokens      Cost",
	);
	console.log("-".repeat(120));
	for (const r of results) {
		const line =
			...
			" " +
			String(r.conc).padStart(6) +
			" " +
			String(r.batches).padStart(7) +
			" " +
			`${r.ok}/${r.fail}`.padStart(7) +
			" " +
			`${r.ids}/${r.reqs}`.padStart(6) +
			" " +
			(r.ms / 1000).toFixed(1).padStart(7) +
			"s " +
			String(r.tk).padStart(7) +
			" $" +
