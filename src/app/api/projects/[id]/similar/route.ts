...
// ── DB 행 타입 ────────────────────────────────────────────
type ProjRow = { org_id: string };
type SourceReqRow = { id: string; original_id: string | null; name: string | null; source_text: string };
type DocRow = { project_name: string | null; period: string | null; name: string | null; header_text: string | null };
type TopTypeRow = { type: string };
type TargetDetailRow = { project_id: string; name: string; period: string | null; doc_name: string; header_text: string | null; req_count: string };
type HeaderResultRow = { target_project_id: string; target_project_name: string; header_score: string; header_headline: string };
type SelfRankRow = { self_rank: number };
type PairRow = { source_id: string; source_oid: string | null; source_name: string | null; target_id: string; target_oid: string | null; target_name: string | null; target_text: string; target_headline: string; target_project_id: string; target_project_name: string; score: number; match_type: string };
...
function extractKeywords(text: string): string[] {
	...
	const matchs = text.match(/[A-Za-z]{2,}/g) || [];
	for (const t of matchs) words.add(t);
	return Array.from(words).slice(0, 15);
}

// ── 보일러플레이트 제거 (표시용) ─────────────────────────────
function _stripBoilerplate(text: string): string {
	return text
		.replace(
...
export async function GET(
		...
		const extBizName = (ht: string) => {
			...
			while ((m = p.exec(ht)) !== null) {
				...
				if (c.length >= 5 && !/^[·.\s\d]+$/.test(c))
					return c.replace(/\t/g, "").replace(/\s{2,}/g, " ");
			}
    ...
    `);
		...
		const src = sourceDoc as unknown as DocRow;
		const sourceInfo = {
			...
			topType: (sourceTopType.rows?.[0] as unknown as TopTypeRow)?.type || "",
		};
      ...
      WHERE p.org_id = ${(proj as ProjRow).org_id}::uuid AND p.id != ${id}::uuid
    `)
			).rows ?? [];

		const targetInfoMap: Record<string, { bizName: string; period: string; docName: string; reqCount: number }> = {};
		for (const td of targetDetails as TargetDetailRow[]) {
		...
		(sourceReqs as unknown as SourceReqRow[]).map((r) => r.source_text).join(" "),
		);

		// 요구사항별 OR tsquery (공백 이스케이프 포함)
		const sourceRows = (sourceReqs as unknown as SourceReqRow[])
			.map((r) => {
		...
		const orgId = (proj as ProjRow).org_id;
		const hasSourceQueries = sourceRows.length > 0;

		// ── 4a. 문서 헤더 비교 (문서수준 유사도) ──
		// biome-ignore lint/suspicious/noExplicitAny: DB execute returns Record<string, unknown>
    ...
    `)
		...
		if (currentDoc && (currentDoc as unknown as DocRow).header_text) {
			const headerText = ((currentDoc as unknown as DocRow).header_text) || "";
			const headerKw = extractKeywords(headerText);
			if (headerKw.length >= 2) {
				headerKwStr = headerKw
					.map((w) => `'${w.replace(/'/g, "''")}'`)
					.join(" | ");
        ...
        `)
					).rows ?? [] as unknown as HeaderResultRow[];
			}
		}

		// ── 자기 자신과의 ts_rank 계산 (정규화 기준점, 동일 문서 = 100%) ──
		let headerSelfScore = 0;
		if (headerKwStr && (currentDoc as unknown as DocRow).header_text) {
			const selfRow =
				(
					await db.execute(sql`
        SELECT ts_rank(
          to_tsvector('simple', ${(currentDoc as unknown as DocRow).header_text}),
          to_tsquery('simple', ${headerKwStr})
        )::numeric AS self_rank
      `)
				).rows ?? [] as unknown as HeaderResultRow[];
			const selfRankRaw = Number((selfRow[0] as unknown as SelfRankRow)?.self_rank || 0);
    ...
    `)
		...
		const projectGroups: Record<
			string,
			{
				...
				idPairs: Array<{sourceId: string; sourceOriginalId: string | null; sourceName: string | null; targetId: string; targetOriginalId: string | null; targetName: string | null; targetText: string; targetHeadline: string; matchType: string; score: number}>;
				contentPairs: Array<{sourceId: string; sourceOriginalId: string | null; sourceName: string | null; targetId: string; targetOriginalId: string | null; targetName: string | null; targetText: string; targetHeadline: string; matchType: string; score: number}>;
		...
		// 헤더 비교 결과로 그룹 시드 생성
		for (const hr of headerResults as HeaderResultRow[]) {
			...
			projectGroups[pid] = {
				...
				headerScore:
					headerSelfScore > 0
						? Math.min(
								100,
								Math.round((Number(hr.header_score) / headerSelfScore) * 100),
							)
						: Number(hr.header_score),
		...
		// 요구사항 페어 추가
		for (const row of allPairs as PairRow[]) {
		...
		const totalSource = sourceReqs.filter(
			(r) => r.source_text?.length > 0,
		).length;
		...
		const similar = Object.values(projectGroups)
			.map((g) => {
				...
				const idOverlap =
					totalSource > 0 ? Math.round((idMatchCount / totalSource) * 100) : 0;

				// 중복 제거 페어 병합: content (점수순) → id
				const seenPairs = new Set<string>();
				const mergedPairs: Array<{sourceId: string; targetId: string; matchType: string; score: number}> = [];

				for (const p of [
					...g.contentPairs.sort((a, b) => b.score - a.score),
					...g.idPairs,
				]) {
