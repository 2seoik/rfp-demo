...
>
> 이 문서는 프로젝트의 단일 진실 공급원(single source of truth)이다. §16(알려진 문제)과 §17(개선 우선순위)이 현재 상태를 추적하며, 작업 완료 시 ✅/⚠️/🔴로 갱신한다.
...
정렬+필터+a11y 완료.
### 16.19 분석 진행률 구간 불균형 (UX P2) 🔴

§8.6의 진행률 단계에서 "AI 분석"(20~70%) 구간이 실제 분석 시간의 대부분을 차지한다. 청크 완료마다 "N/59개 처리 중"으로 갱신하면 사용자가 멈춤으로 오인하는 것을 방지할 수 있다.

### 16.20 ID 기반 추출 커버리지 검증 미구현 (품질 P2) 🔴

§18.2의 `coverage = 저장된 ID 수 / 원문 탐지 ID 수` 검증이 구현되지 않았다. coverage < 0.85인 문서/청크만 느린 고품질 모델로 재처리하는 선택적 전략으로 비용 효율적 개선 가능.

### 16.21 DB 백업 전략 부재 (운영 P2) 🔴

PostgreSQL 데이터 백업 및 복구 절차가 정의되지 않았다. `pg_dump` 스크립트와 복구 테스트를 문서화한다. §19.5의 TRUNCATE 초기화만 있고 백업 절차는 없다.
...
| 17.22 | 분석 진행률 구간 세분화 | 청크 완료마다 "N/59" 카운터 표시 | 🔴 | 16.19 |
| 17.23 | ID 기반 coverage 검증 | coverage < 0.85 시 고품질 모델 재처리 | 🔴 | 16.20 |
| 17.24 | DB 백업 전략 문서화 | pg_dump 스크립트 + 복구 테스트 | 🔴 | 16.21 |
